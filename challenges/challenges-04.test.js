...
const isNum = ((input) => {
  let x = input;
  const regexThing = /\d+/g;
  const thing = regexThing.test(x);
  return thing;
});
...
const isCapitalized = (str) => {
  const wordRegex = /[A-Z]\w+/g;
  const foudn = str.match(wordRegex);
  if (foudn) {
    return foudn;
  } else {
    return [];
...
const citiesAtoJ = (arr) => {
  ...
  arr.forEach((object, index) => {
    let x = arr[index].match(reggi);
    if (x) {
      returnArray.push(x[0]);
    }
  });
  return returnArray;
};
