...
// https://regexlib.com/REDetails.aspx?regexp_id=50
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const validateEmail = (email) => {
  const reggi = /^\w+.?\w+@\w+\.(com|org|net)$/g;
  // bade (TA) assisted myself and Dar-ci determining the logic for this question
  if (reggi.test(email)) {
    return true;
  } else {
    return false;
...
const validatePhoneNumber = (phoneNumber) => {
  const reggi = /^\D?(\d{3})\D?\D?(\d{3})\?(\d{4})$/g;
  // hmm
  if (reggi.test(phoneNumber)) {
    return true;
  } else {
    return false;
  }
