...
const templateWithJQuery = () => {
  // const $grabTemp = $('#template').clone();
  starWarsPeople.forEach((object, index) => {
    const $grabTemp = $('section:first-child').clone();
    const $h2 = $grabTemp.find('h2');
    $h2.text(object.name);
    const $h3 = $grabTemp.find('h3');
    $h3.text(object.height);
    $grabTemp.find('p').text(object.eye_color);
    $('main').append($grabTemp);
...
const listFoods = (recipe) => {
  let result = [];
  const useful = recipe.ingredients;
  useful.forEach((object, index, array) => {
    const x = object.indexOf(' ') + 1;
    // console.log(x);
    const y = object.indexOf(' ', x);
    result.push(object.slice(y + 1, object.length));
