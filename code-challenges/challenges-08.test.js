...
const oddValues = (arr) => {
  const returnArry = arr.filter((value, index, array) => {
    if (value % 2) {
      return true;
    } else {
      return false;
...
const filterStringsWithVowels = (arr) => {
  ...
  const reggi = /[aeiou]/;
  const returnArray = arr.filter((obj, index, array) => {
    if (reggi.test(obj) === true) {
      return true;
    } else {
      return false;
    }
...
const notInFirstArray = (forbiddenValues, arr) => {
  const output = arr.filter((value, index, array) => {
    let x = false;
